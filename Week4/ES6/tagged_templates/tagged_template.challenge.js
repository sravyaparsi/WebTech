...
function checkQuery() {
    ...
    let finalQuery=tag`${command} FROM ${table} WHERE ${whereClauses[0]} ${whereClauses[1]} ORDER BY asc`
    
    return (finalQuery);
}
function tag(strings,commandExp,tableExp,uidExp,cityExp ){
    
    if(commandExp.includes("SELECT") || commandExp.includes("UPDATE")){
        console.log(`${strings[1]}`)
        return `${commandExp} ${strings[1]}${tableExp}${strings[2]}${uidExp}${cityExp}${strings[4]}`
    }
}
